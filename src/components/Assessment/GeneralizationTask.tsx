/**
 * ENG-021: A-3 Generalization — submit 2 different equivalents.
 * Max 3 attempts total across both submissions.
 */

import { useState } from 'react';
import type { A3Generalization } from '../../state/types';
import type { Fraction } from '../../engine/FractionEngine';
import { areEquivalent, simplify } from '../../engine/FractionEngine';
...
export function GeneralizationTask({
  problem,
  attempts,
  blocks,
  selectedBlockId,
  ...
  onAnswer,
  onAdvance,
}: GeneralizationTaskProps) {
  const [firstAnswer, setFirstAnswer] = useState<Fraction | null>(null);
  ...
  const needFirst = firstAnswer === null;
  ...
  const handleSubmit = () => {
    ...
    const simplifiedAnswer = simplify(answer);
    const simplifiedTarget = simplify(problem.target);
    const sameAsTarget =
      simplifiedAnswer.numerator === simplifiedTarget.numerator &&
      simplifiedAnswer.denominator === simplifiedTarget.denominator;

    if (!equivalent || sameAsTarget) {
      const atMax = attempts + 1 >= MAX_ATTEMPTS;
      onAnswer(false);
      if (atMax) onAdvance();
      else
        setRejectionMessage(
          sameAsTarget
            ? 'That’s the same as the target — try a different fraction!'
            : 'Not equivalent — try again!'
        );
      return;
    }
    ...
    if (needFirst) {
      ...
      setRejectionMessage(null);
      dispatch({ type: 'RESET_ASSESSMENT_WORKSPACE' });
    } else {
      const sameDenom =
        simplify(answer).denominator === simplify(firstAnswer).denominator;
      if (sameDenom) {
        setRejectionMessage(
          'Use a different denominator than your first answer!'
        );
        return;
      }
      onAnswer(true);
  ...
  const handleSplitRequest = (parts: number) => {
    ...
    setRejectionMessage(null);
    const newIds = Array.from({ length: parts }, (_, i) => `block-${nextBlockId + i}`);
