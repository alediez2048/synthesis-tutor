/**
 * ENG-020: Assessment problem pools per PRD Section 7.4.
 * Data defined in TypeScript (matches exploration-config.ts pattern).
 */

import type {
  A1Recognition,
  A2Construction,
  A3Generalization,
  AssessmentProblem,
} from '../state/types';

// ── A-1: Recognition (Multiple Choice) ──────────────────────────
const a1Sets: Omit<A1Recognition, 'id' | 'type'>[] = [
  {
    target: { numerator: 1, denominator: 2 },
    options: [
      { fraction: { numerator: 2, denominator: 4 }, correct: true },
      { fraction: { numerator: 1, denominator: 3 }, correct: false },
      { fraction: { numerator: 3, denominator: 4 }, correct: false },
      { fraction: { numerator: 2, denominator: 3 }, correct: false },
    ],
    maxAttempts: 2,
  },
  {
    target: { numerator: 1, denominator: 3 },
    options: [
      { fraction: { numerator: 2, denominator: 6 }, correct: true },
      { fraction: { numerator: 1, denominator: 2 }, correct: false },
      { fraction: { numerator: 2, denominator: 4 }, correct: false },
      { fraction: { numerator: 3, denominator: 4 }, correct: false },
    ],
    maxAttempts: 2,
  },
  {
    target: { numerator: 3, denominator: 4 },
    options: [
      { fraction: { numerator: 6, denominator: 8 }, correct: true },
      { fraction: { numerator: 2, denominator: 3 }, correct: false },
      { fraction: { numerator: 1, denominator: 2 }, correct: false },
      { fraction: { numerator: 5, denominator: 6 }, correct: false },
    ],
    maxAttempts: 2,
  },
];

// ── A-2: Construction ───────────────────────────────────────────
const a2Sets: Omit<A2Construction, 'id' | 'type'>[] = [
  {
    target: { numerator: 1, denominator: 2 },
    startingBlock: { numerator: 1, denominator: 1 },
    maxAttempts: 3,
  },
  {
    target: { numerator: 2, denominator: 3 },
    startingBlock: { numerator: 1, denominator: 1 },
    maxAttempts: 3,
  },
  {
    target: { numerator: 1, denominator: 4 },
    startingBlock: { numerator: 1, denominator: 2 },
    maxAttempts: 3,
  },
];

// ── A-3: Generalization ─────────────────────────────────────────
const a3Sets: Omit<A3Generalization, 'id' | 'type'>[] = [
  {
    target: { numerator: 1, denominator: 2 },
    requiredCount: 2,
  },
  {
    target: { numerator: 1, denominator: 3 },
    requiredCount: 2,
  },
];

function pickRandom<T>(arr: T[]): T {
  return arr[Math.floor(Math.random() * arr.length)]!;
}

/**
 * Select one random problem set for each assessment type.
 * Called once at session start to populate assessmentPool.
 */
export function selectAssessmentProblems(): AssessmentProblem[] {
  const a1 = pickRandom(a1Sets);
  const a2 = pickRandom(a2Sets);
  const a3 = pickRandom(a3Sets);

  return [
    { id: 'a1', type: 'recognition', ...a1 },
    { id: 'a2', type: 'construction', ...a2 },
    { id: 'a3', type: 'generalization', ...a3 },
  ];
}
